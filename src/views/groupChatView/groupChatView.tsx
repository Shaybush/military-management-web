...
const GroupChatView: FC = () => {
  ...
  const [userData, setUserData] = useState<IUserDataModel["data"]>(
  ...
  useEffect(() => {
    const messages =
      localStorage["messages"] && JSON.parse(localStorage["messages"]);
  ...
  const onServerListen = (_item: IMessageModel): void => {
    if (!isStringEmptyUtil(_item.msg)) {
      setAllMessage((prev: Array<IMessageModel>) => [
        ...prev,
        { ..._item, msg: _item.msg.trim() },
      ]);
    }
  };

  const onMessageDelete = (_messageId: string): void => {
    setAllMessage((prev: IMessageModel[]) =>
      prev.filter((message: IMessageModel) => message.msg_id != _messageId)
    );
    localStorage.setItem(
      "messages",
      JSON.stringify(
        allMessage.filter(
          (message: IMessageModel) => message.msg_id != _messageId
        )
      )
    );
  };
                    ...
                    allMessage[i]?.userId !== allMessage[i - 1]?.userId ? (
                      ...
                      >
                        {item.userId !== userData.id && (
                          <img
                            src={item.img}
                            className="rounded-circle user-image-profile me-1"
                          />
                        )}
                        <DropdownButton
                        ...
                        {item.userId === userData.id && (
                          <img
                            src={item.img}
                            className="rounded-circle user-image-profile"
                          />
                        )}
                      </div>
                    ) : (
