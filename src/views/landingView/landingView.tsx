...
const LandingView: FC = () => {
    ...
    useEffect(() => {
      const isValid = async () : Promise<void> => {
        if (user && user.access_token) {
          const response = await getUserDetailsFromAccessToken();
          if (response) {
            setUserDetailsInLocalStorage(response);
            navigate("/group-chat")
          }
        }
      };
    ...
    const login = useGoogleLogin({
      onSuccess: (codeResponse: ILandingModel) => { 
    ...
    const setUserDetailsInLocalStorage = (userDetails: IUserDataModel) : void => {
